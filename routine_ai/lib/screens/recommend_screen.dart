...
class RecommendScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: const RecommendChatPanel(),
    );
  }
}

class RecommendChatPanel extends StatefulWidget {
  ...
  @override
  State<RecommendChatPanel> createState() => _RecommendChatPanelState();
}

class _RecommendChatPanelState extends State<RecommendChatPanel> {
      ...
      child: Column(
        ...
        children: [
          ...
          if (_plans.isEmpty && !_isLoading)
            ...
            ..._plans.map(
              (plan) =>
                  _PlanCard(plan: plan, onAdd: () => _handleAddPlan(plan)),
            ),
          ],
        ],
      ),
  ...
  Future<void> _requestRecommend() async {
    ...
    try {
      ...
      coachReply = _fallbackCoachReply();
      newPlans = _fallbackPlans();
    } finally {
      if (mounted) {
  ...
  String _composeUserMessage(String freeText) {
    ...
    buffer.writeln('위 조건으로 추천 루틴을 제안해 주세요.');
  ...
  List<RoutinePlan> _fallbackPlans() {
    return [
      RoutinePlan(
        title: '아침 15분 전신 루틴',
        ...
        durationMinutes: 15,
        difficulty: 'easy',
        reason: '기상 직후 가볍게 전신을 풀어 집중력을 끌어올려요.',
      ),
      RoutinePlan(
        title: '출근 전 코어 강화',
        days: const ['월', '수', '금'],
        ...
        durationMinutes: 20,
        difficulty: 'mid',
        reason: '플랭크와 스쿼트 중심이라 매일 해도 무리가 적어요.',
      ),
      RoutinePlan(
        title: '굿모닝 스트레칭',
        days: const ['월', '화', '수', '목', '금', '토', '일'],
        ...
        durationMinutes: 10,
        difficulty: 'easy',
        reason: '주말에도 가볍게 몸을 깨워 리듬을 유지해요.',
      ),
    ];
  }

  String _fallbackCoachReply() {
    return '아침에 꾸준히 움직이면 하루 에너지가 달라져요!, 추천 루틴을 참고해 건강한 습관을 만들어보세요.';
  }

  Future<void> _handleAddPlan(RoutinePlan plan) async {
    final recommendation = RoutineRecommendation(
      title: plan.title,
      iconKey: _iconKeyForPlan(plan.difficulty),
      time: plan.time,
      days: plan.days.isEmpty ? ['월', '수', '금'] : plan.days,
    ...
    final error = await appState.createRoutine(
      title: result.title,
      time: result.time,
      days: result.days,
    ...
    if (error != null) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('루틴이 추가되었습니다.')));
    }
  }

  String _iconKeyForPlan(String difficulty) {
    switch (difficulty) {
      case 'hard':
        return 'run';
      case 'mid':
...
class _PlanCard extends StatelessWidget {
  const _PlanCard({required this.plan, this.onAdd});
          ...
          children: [
            Row(
              ...
              children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '시간: ${plan.time} · 소요 ${plan.durationMinutes}분 · 난이도 ${_difficultyLabel(plan.difficulty)}',
                      ),
                      const SizedBox(height: 4),
                      Text('추천 요일: $slotText'),
                    ],
                ...
                TextButton(
                  onPressed: onAdd ?? () => _showComingSoon(context),
                  child: const Text('추가'),
                ),
              ],
            ),
  ...
  void _showComingSoon(BuildContext context) {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('추가 기능은 준비중입니다.')));
  }
}
